...
const PostApplication = () => {
  ...
  const handlePostApplication = (e) => {
    ...
    if (resume) { //ki agr user ne resume bheja to, me ye condition isliye lga rha hu knyu ho shakta hai user ne register hote waqt resume add kr diya ab wapas nahi beja ho 
  ...
  useEffect(() => {
    if (user) {
      setName(user.name || ""), //yaani agr user hai to setName ko user.name krdo ya(||) agr user nhai hai to use ek empty string bna do , same sub me
        setEmail(user.email || ""),
        setPhone(user.phone || ""),
    ...
    if (message) {
      ...
      dispatch(resetApplicationSlice()); // reset aplication krvadenge taki bar bar message nahi aayega 
    }

    dispatch(fetchSingleJob(jobId));
  }, [dispatch, error, message, jobId, user]);

  ////////////////hamare jobSchema me hamare pass ye qualifications responsibilities offers hai in tino ko me ek aaray me convert kr lunga by . fullstop
  let qualifications = [];
